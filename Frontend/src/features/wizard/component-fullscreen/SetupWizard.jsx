        ...
        <div className="custom-scrollbar recommendation-panel__body">
            {loading ? (
                <div className="panel-loading">
                    <div className="panel-spinner" style={{ borderColor: `${accentColor}30`, borderTopColor: accentColor }} />
...
const SetupWizard = ({ onFinish }) => {
    ...
    const handleStartProject = (selectedProducts) => {
        ...
        if (selectedSetup?.id) {
            navigate(`/builder?setupId=${encodeURIComponent(selectedSetup.id)}`);
        } else {
            navigate('/builder');
        }
        if (onFinish) onFinish(selectedProducts, selectedSetup);
    };
