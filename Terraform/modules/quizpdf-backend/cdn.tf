...
resource "aws_cloudfront_distribution" "backend" {
  ...
  origin {
    domain_name = replace(aws_apigatewayv2_stage.prod.invoke_url, "/^https?://([^/]*).*/", "$1")
  ...
  default_cache_behavior {
    ...
    # Backend-ul nu trebuie cache-uit de CloudFront
    min_ttl     = 0
    default_ttl = 0
    max_ttl     = 0
  }
...
# Record-ul pentru ROOT (Frontend)
...
resource "aws_route53_record" "api" {
  zone_id = var.hosted_zone_id
  name    = "api.${var.domain_name}" # ADAUGĂ "api." și aici
  type    = "A"
  alias {
