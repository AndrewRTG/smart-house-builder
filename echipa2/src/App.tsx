...
import { splitWallByOpening } from './wallSplit';
...
const App: React.FC = () => {
  ...
  const [selectedDevice, setSelectedDevice] = useState<{ id: string, name: string, brand: string, type: string, status: string, priceEUR: number } | null>(null);
  ...
  const catalogDevices = [
    { id: '1', name: 'Philips Hue E27', price: '49€', priceEUR: 49, brand: 'Philips', type: 'bec', status: 'online' },
    { id: '2', name: 'Nest Thermostat', price: '279€', priceEUR: 279, brand: 'Google', type: 'senzor', status: 'online' },
    { id: '3', name: 'Lock', price: '30€', priceEUR: 30, brand: 'Amazon', type: 'lock', status: 'online' },
    { id: '4', name: 'Router', price: '200€', priceEUR: 200, brand: 'Amazon', type: 'router', status: 'online' },
    { id: '5', name: 'Ps5 ', price: '400€', priceEUR: 400, brand: 'Sony', type: 'controller', status: 'online' },
    { id: '6', name: 'Smart Tv ', price: '638€', priceEUR: 638, brand: 'Samsung', type: 'tv', status: 'online' },
    { id: '7', name: 'Door Camera', price: '64€', priceEUR: 64, brand: 'Amazon', type: 'interfon', status: 'online' },
    { id: '8', name: 'Extension Cord', price: '15€', priceEUR: 15, brand: 'Amazon', type: 'prelungitor', status: 'online' },
    { id: '9', name: 'Sound system ', price: '148€', priceEUR: 148, brand: 'Amazon', type: 'soundsystem', status: 'online' },
    { id: '10', name: 'Plug', price: '5€', priceEUR: 5, brand: 'Amazon', type: 'priza', status: 'online' },
    { id: '11', name: 'Smart vacum', price: '250€', priceEUR: 250, brand: 'Amazon', type: 'aspirator', status: 'online' },
    { id: '12', name: 'Hub', price: '300€', priceEUR: 300, brand: 'Amazon', type: 'hub', status: 'online' }
  ];
  ...
  const handleCanvasClick = (col: number, row: number) => {
    ...
    if (!exists) {
      setPlacedIcons([...placedIcons, {
        ...
        priceEUR: selectedDevice.priceEUR
      }]);
    }
  };
    ...
    if (t === 'window' || t === 'door') {
      setLines((prevLines) => {
        for (let idx = 0; idx < prevLines.length; idx++) {
          const line = prevLines[idx];
          if (line.type !== 'wall') continue;
          const split = splitWallByOpening(line.start, line.end, start, end);
          if (!split) continue;
          const withoutWall = prevLines.filter((_, i) => i !== idx);
          const additions: typeof prevLines = [];
          if (split.before) {
            additions.push({ id: newLineId(), type: 'wall', start: split.before.start, end: split.before.end });
          }
          if (split.after) {
            additions.push({ id: newLineId(), type: 'wall', start: split.after.start, end: split.after.end });
          }
          additions.push({ id: newLineId(), type: t, start, end });
          return [...withoutWall, ...additions];
        }
        return [...prevLines, { id: newLineId(), type: t, start, end }];
      });
      return;
    }
    setLines((prevLines) => [...prevLines, { id: newLineId(), type: t, start, end }]);
  };
